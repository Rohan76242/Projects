...
def load_audio(path):
    ...
    if sr != SAMPLE_RATE:
        print(
            f"Resampling {sr} Hz -> {SAMPLE_RATE} Hz..."
        )
    ...
    # Match train.py's normalize_audio:
    ...
    print(
        f"Audio samples: {waveform.shape[1]}"
    )
...
def test_audio(model, audio_path, device):
    ...
    print(
        f"Network input shape: {features.shape}"
    )
    ...
    with torch.no_grad():
        ...
        wake_probability = probabilities[
            0,
            1
        ].item()

    if wake_probability >= 0.85:
    ...
    print(
        f"Wake probability: {wake_probability:.2f}%"
    )
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(
            'Usage: python test.py "C:\\path\\to\\audio.wav"'
        )
        ...
        print(
            r'python test.py "C:\sobia\wakeword\positive\sobia_001.wav"'
        )
